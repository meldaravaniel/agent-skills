...
└── evals/ (optional)
    ...
    └── fixtures/          - Read-only input fixtures (never written to by evals)
```

If a skill writes files as part of its work, eval runs must write output to `workspace/<iteration-id>/<eval-id>/with_skill/outputs/` (or `without_skill/outputs/`) — never to `evals/fixtures/`, which is for read-only input fixtures only.
...
Put results in `workspace/` inside the skill directory. Add `workspace/` to the root `.gitignore` of the repository if it isn't already there. Within the workspace, organize results by iteration (`iteration-1/`, `iteration-2/`, etc.) and within that, each test case gets a directory (`eval-0/`, `eval-1/`, etc.). Don't create all of this upfront — just create directories as you go.

### Step 1: Spawn all runs (with-skill AND baseline) in the same turn

Before spawning, create the output directories. For every eval that has entries in its `files` array, copy those fixtures into both config output directories:

```bash
mkdir -p <skill-dir>/workspace/<iteration-id>/<eval-id>/with_skill/outputs
mkdir -p <skill-dir>/workspace/<iteration-id>/<eval-id>/without_skill/outputs
cp <skill-dir>/evals/fixtures/<filename> <skill-dir>/workspace/<iteration-id>/<eval-id>/with_skill/outputs/
cp <skill-dir>/evals/fixtures/<filename> <skill-dir>/workspace/<iteration-id>/<eval-id>/without_skill/outputs/
```

This gives each config an isolated working copy of any fixtures it needs. The skill reads from and writes to its own `outputs/` directory, and `evals/fixtures/` is never touched.

Then spawn two subagents in the same turn for each test case — one with the skill, one without. Don't spawn the with-skill runs first and come back for baselines later; launch everything at once so it all finishes around the same time.

Eval prompts in `evals.json` are iteration-agnostic — they don't include output paths. When spawning each subagent, append the outputs path to the prompt:

> Save the output to: `<skill-dir>/workspace/<iteration-id>/<eval-id>/with_skill/outputs/`

This keeps `evals.json` reusable across iterations.
